...
$msg = '';
//cek apakah request berasal dari form post dan apakah ada data selected_mk
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['selected_mk'])) {
    ...
    if (!empty($selected)) { //kalau ada matkul yang dipilih
        $placeholders = implode(',', array_fill(0, count($selected), '?')); //menggabung kode-kode matakuliah yang dipilih menjadi array of string
        $params = array_merge([trim($id_sem)], $selected); //gabungkan semester dan daftar mata kuliah
        //query untuk mengambil id mk, nama mk, sks mk, dan jadwalnya menggunakan subquery di FROM
        $stmt2 = $conn->prepare("
            ...
            WHERE j.Id_MK = mk.Id_MK
              AND j.Id_Sem = ?
              AND mk.Id_MK IN ($placeholders)
        ");
        $stmt2->execute($params);
        $jadwalSelected = $stmt2->fetchAll(PDO::FETCH_ASSOC);//ambil semua jadwal matkul yang dipilih

        try {
            $conn->beginTransaction(); //mulai transaction database
            $id_frs = substr($npm, -4) . $id_sem;// buat id frs dari 4 digit terakhir npm + id semester
            $stmt2 = $conn->prepare("SELECT Id_FRS FROM FRS WHERE Id_FRS = ?"); //cek apakah frs udh ada
            $stmt2->execute([$id_frs]);
            if (!$stmt2->fetch()) {
                $conn->prepare("INSERT INTO FRS (Id_FRS, NPM) VALUES (?, ?)")->execute([$id_frs, $npm]);
            }

            //simpan enroll lama ke History_FRS sebelum dihapus (INSERT...SELECT subquery DML)
            //hanya simpan kalau memang ada enroll lama
            $cekEnroll = $conn->prepare("SELECT COUNT(*) FROM Enroll WHERE NPM = ? AND Id_Sem = ?");
            $cekEnroll->execute([$npm, $id_sem]);
            if ($cekEnroll->fetchColumn() > 0) {
                //ambil versi terakhir + 1
                $versiStmt = $conn->prepare("
                    SELECT ISNULL(MAX(Versi), 0) + 1 FROM History_FRS WHERE NPM = ? AND Id_Sem = ?
                ");
                $versiStmt->execute([$npm, $id_sem]);
                $versi = $versiStmt->fetchColumn();

                //INSERT...SELECT: copy enroll lama ke history dengan versi baru
                $conn->prepare("
                    INSERT INTO History_FRS (NPM, Id_Sem, Id_MK, Versi, Disimpan_pada)
                    SELECT NPM, Id_Sem, Id_MK, ?, Dibuat_pada
                    FROM Enroll
                    WHERE NPM = ? AND Id_Sem = ?
                ")->execute([$versi, $npm, $id_sem]);
            }

            //hapus enroll lama menggunakan subquery DML - delete berdasarkan FRS milik mahasiswa
            $conn->prepare("
                DELETE FROM Enroll 
                WHERE Id_FRS IN (SELECT Id_FRS FROM FRS WHERE NPM = ?) 
                  AND Id_Sem = ?
            ")->execute([$npm, $id_sem]);

            //insert enroll baru menggunakan subquery DML - ambil Id_FRS dari tabel FRS
            $ins = $conn->prepare("
                INSERT INTO Enroll (NPM, Id_MK, Id_Sem, Id_FRS, Dibuat_pada) 
                VALUES (?, ?, ?, (SELECT Id_FRS FROM FRS WHERE NPM = ? AND Id_FRS = ?), GETDATE())
            ");
...
//menggunakan subquery di SELECT untuk nama dosen
$stmt = $conn->prepare("
    SELECT j.Id_Jadwal, mk.Id_MK, mk.Nama AS NamaMK, mk.SKS, j.Hari, j.Jam_Mulai, j.Jam_Selesai,
...
//ambil mata kuliah yang sudah diambil di enroll menggunakan query biasa
