...
$id_sem = '26-1'; //variabel untuk menentukan semester mana frs bisa dilakukan
...
//mengambil hari, jam, mata kuliah, ruangan, dosen, sks yang berlangsung
//menggunakan subquery di SELECT untuk nama matakuliah, sks, dan nama dosen
$stmt = $conn->prepare("
    SELECT j.Hari, j.Jam_Mulai, j.Jam_Selesai, j.Id_MK, j.Ruangan,
           (SELECT Nama FROM MataKuliah WHERE Id_MK = j.Id_MK) AS NamaMK,
           (SELECT SKS FROM MataKuliah WHERE Id_MK = j.Id_MK) AS SKS,
...
function fmtTime($t){ return substr($t,0,5); } //ambil jam dan menit saja untuk waktu mulai dan selesai
?>
